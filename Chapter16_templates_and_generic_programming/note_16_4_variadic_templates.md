...
* **参数绑定机制**：
  ...
  * 但递归调用只传递**两个实参(two arguments)**：`os`和`rest...`
...
* **匹配情况**：两个版本的`print`都可行(viable)：
  * 可变参数版本：可以匹配，因为参数包可以为空
  * 非可变参数版本：传递了两个实参，第一个实参类型是`ostream`，完全匹配
* **选择结果**：调用恰好传递了两个实参，非可变参数版本是可行的(viable)。
* **关键洞察**：即使可变参数版本也可行，**非可变参数模板(nonvariadic template)**比**可变参数模板(variadic template)**更特化，因此选择非可变参数版本（§ 16.3, p. 695）。
...
**理论**
* **与常规参数的区别**：与普通实参不同，**参数包可以为空(parameter pack can be empty)**。
* **实例化能力**：因此，`print`的可变参数版本可以只用**两个参数(two parameters)**实例化：一个用于`ostream&`参数，另一个用于`const T&`参数。
...
* **特化程度规则**：然而，**非可变参数模板(nonvariadic template)**比**可变参数模板(variadic template)**更特化(more specialized)。
* **选择结果**：因此，对于此调用选择**非可变参数版本(nonvariadic version)**（§ 16.3, p. 695）。

**注意点**
* 📋 **术语对照**：more specialized（更特化）、nonvariadic template（非可变参数模板）
...
**理论**
* **警告内容**：当定义可变参数版本时，非可变参数版本的`print`声明**必须在作用域内(must be in scope)**。否则，可变参数函数将**无限递归(recurse indefinitely)**。
...
**理论**
* **本节主题**：16.4.2节介绍**参数包展开(Pack Expansion)**。
* **展开操作**：除了获取参数包的大小外，我们能对参数包做的唯一另一件事就是**展开(expand)**它。
* **展开定义**：当我们展开一个包时，还需要提供一个**模式(pattern)**用于每个展开的元素。展开一个包将其分解为**组成元素(constituent elements)**，将模式分别应用于每个元素。

**注意点**
* 📋 **术语对照**：pack expansion（参数包展开）、pattern（模式）、constituent elements（组成元素）
...
**理论**
* **触发展开**：通过在模式的右侧放置**省略号(ellipsis)** `...`来触发展开。
...
**理论**
* **Args展开**：`Args`的展开将模式`const Args&`应用于模板参数包`Args`中的每个元素。
* **展开结果**：此展开的结果是**零个或多个参数类型(zero or more parameter types)**的逗号分隔列表，每个都具有`const type&`的形式。
