...
**理论**
* **核心概念**：函数模板可以被其他模板或普通非模板函数重载
* **基本原则**：与普通函数一样，同名函数必须在**参数数量**或**参数类型**上有所不同
* **技术本质**：重载决议机制同样适用于模板函数
...
**理论**
函数匹配(§6.4)受函数模板存在的影响，具体表现为以下四个方面：

1. **候选函数包含模板实例化**：调用的候选函数集合包括任何<b>模板参数推导(§16.2)</b>成功的函数模板实例化

2. **候选模板总是可行的**：候选函数模板总是可行的，因为模板参数推导会排除不可行的模板

3. **可行函数按转换分级排序**：与普通函数一样，可行函数（包括模板和非模板）按调用所需的转换（如有）进行分级。当然，用于调用函数模板的转换**相当有限**(§16.2.1)
...
* 🔄 **知识关联**：这与(§16.2.1)中讨论的模板参数转换限制直接相关
...
**注意点**
* ⚠️ **重要警告**：此函数对`char*`不能正常工作，将在§16.3 讨论
...
* **IO库行为**：该版本的`<<`假定指针表示以空字符结尾的字符数组，打印数组内容而非地址
* **后续处理**：§16.3将介绍如何处理字符指针
...
* **第二个匹配失败**：第二个版本要求指针参数，而此次调用传递的是非指针对象
...
* 💡 **规则必要性**：此规则的原因在于，否则将无法在指向`const`的指针上调用`debug_rep`的指针版本
...
* **匹配分析**：
  - 两个模板对参数都是精确匹配，第二个模板需要数组到指针的转换，该转换被视为**函数匹配目的的精确匹配**（§6.6.1）
  - 非模板版本可行，但需要**用户定义转换**
...
* **静默实例化(silently instantiate)**：如果编译器可以从模板实例化调用，则缺失的声明无关紧要
* **具体示例**：如果忘记声明接受`string`的`debug_rep`版本，编译器将<b>静默实例化</b>接受`const T&`的模板版本
