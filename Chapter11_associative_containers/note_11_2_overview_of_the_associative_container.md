...
* [*知识点5: 键类型的要求*](#id6)
* [*知识点6: 有序容器的键类型要求与严格弱序*](#id7)
* [*知识点7: 使用自定义比较函数*](#id8)
* [*知识点8: 定义带自定义比较函数的容器*](#id9)
* [*知识点9: pair类型*](#id10)
* [*知识点10: 表11.2 pair的操作*](#id11)
* [*知识点11: 创建和返回pair对象的函数(C++11)*](#id12)
...
**理论**
* 关联容器(associative containers)支持和不支持的一些通用容器操作。
* 第9.2节（p.328）和表9.2（p.330）列出的通用容器操作。
* **不支持的操作**：
    * 不支持顺序容器(sequential container)的位置相关操作，如 `push_front` 或 `back`。
    * 因为元素是基于其<b>键(key)</b>存储的，这些操作对关联容器没有意义。
...
<a id="id4"></a>
## ✅ 知识点4: 初始化`multimap`和`multiset`

**理论**
* `map`/`set`与`multimap`/`multiset`在键**唯一性**上有关键区别
* **键的唯一性**：
...
**注意点**
* ⚠️ 传递给排序算法（§ 10.3.1, p.386）的可调用对象(callable object)必须满足与关联容器中键相同的要求。
...
**理论**
* 可以为有序容器提供自定义的比较操作以替代 `<` 运算符
    * 但该操作必须定义在键类型上的“严格弱序(strict weak ordering)”
...
* **实际意义**：
    * 如果两个键等价，容器将它们视为相等。
    * 在`map`中，将只有一个元素与这些相等键关联，并且任一键都可用于访问对应的值。
...
**理论**
* 操作类型是容器类型的一部分，必须在定义容器类型时在尖括号内提供。创建容器对象时，再将具体的比较操作作为构造函数参数传入。
...
**注意点**
* 🔄 **比较操作的类型**需要是一种**函数指针类型(function pointer type)**。
...
**理论**
* 为了对`multiset<Sales_data>`使用使用自定义比较函数`compareIsbn`，我们需要如下：
    * **类型指定**：需要提供两个类型
        * 键类型(`Sales_data`)
        * 比较类型（指向`compareIsbn`的函数指针类型）使用`decltype`获取函数类型，并加上`*`表示指针
...
**注意点**
* ⚠️ **语法细节**：
    * 使用`decltype(compareIsbn)*`来声明函数指针类型
    * `compareIsbn`和`&compareIsbn`作为构造函数参数效果相同
...
**理论**
* 介绍标准库`pair`类型定义在`utility`头文件中，`pair`是一个模板，包含两个数据成员，类型可以不同。
* **定义与初始化**：
    * **默认构造函数**：值初始化成员。
...
**注意点**
* ⚠️ **重要操作**：`make_pair`可用于方便地创建`pair`对象，编译器会自动推断类型。关系运算符按<b>字典序(lexicographical order)</b>比较。

---

<a id="id12"></a>
## ✅ 知识点12: 创建和返回pair对象的函数(C++11)
