...
**注意点**
* ⚠️ 非成员函数作为类接口的一部分，应与类在同一个头文件中声明
...
<a id="id13"></a>
### ✅ 知识点5.1: 定义read和print函数

**理论**
* read函数从给定流读取数据到给定对象
* print函数将给定对象的内容打印到给定流
* IO类对象不能拷贝，必须通过引用传递
...
**注意点**
* ⚠️ IO类对象不能拷贝，必须通过引用传递
* 💡 read和print函数修改流状态，因此使用普通引用而非const引用
* 🔄 print函数不输出换行，让用户代码控制格式化
...
**理论**
* add函数接受两个Sales_data对象，返回它们的和
* 通过拷贝第一个参数初始化结果，然后使用combine添加第二个参数
...
**注意点**
* ⚠️ 默认情况下，拷贝类对象会拷贝所有数据成员
* 💡 使用combine函数避免了重复的加法逻辑
* 🔄 函数返回sum的拷贝，符合值语义
