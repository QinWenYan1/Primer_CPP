...
* [*知识点7: shrink_to_fit成员函数*](#id12)：请求将capacity减少到与size相等
...
**理论**
* 当向已满的vector添加新元素时，由于**内存连续性要求**，无法在原位置"插入"元素
...
**注意点**
* ⚠️ **性能陷阱**：重分配是vector最昂贵的操作，涉及所有元素
...
**注意点**
* ⚠️ **迭代器失效**：重分配后，所有指向原vector的迭代器、引用和指针都会失效
...
**理论**
* 如果每次调用push_back都触发重分配，时间复杂度将达到O(n²)
* 对于n个元素的vector，总共需要1+2+3+...+n = n(n+1)/2次元素移动
* 这种性能在实际应用中是**不可接受**的

**注意点**
* 🔄 **对比分析**：这正是vector需要超额分配策略的根本原因
...
**理论**
* 为避免频繁重分配，vector和string实现采用**超额分配**（over-allocation）策略
* 当需要新内存时，会分配**超过立即需求**的空间作为**储备**（reserve）
* 后续添加的元素可以直接使用预留空间，无需重分配

**注意点**
* 🎯 **性能优势**：虽然每次重分配仍需移动所有元素，但**重分配次数显著减少**
* 🔄 **反直觉结论**：即使需要移动所有元素，vector通常比list/deque增长更高效
...
**理论**
* vector和string提供专门的成员函数与内存分配部分交互
* Table 9.10列出了三个关键函数：capacity、reserve、shrink_to_fit
...
* **增长方向**：capacity**只增不减**（除非通过shrink_to_fit请求）
* **实现依赖**：具体容量值和增长策略完全由库实现决定

**注意点**
* ⚠️ **重要限制**：调用reserve(n)且n小于当前容量时，capacity不会改变
* 💡 **调试用途**：可以通过观察capacity变化了解实现策略
...
**注意点**
* 🎯 **核心用途**：**预分配内存**，避免多次重分配
* 💡 **使用场景**：已知大致元素数量时，提前reserve可大幅提升性能
...
**理论**
* **不会改变size**：reserve只分配内存，**不添加任何元素**，size保持不变
* **条件触发**：仅在`请求空间 > 当前容量`时生效
* **拒绝缩容**：如果`n <= capacity()`，调用**无任何效果**

**注意点**
* ⚠️ **关键理解**：`size()` == 实际元素数，`capacity()` == 内存可容纳数
* 💡 **记忆口诀**：reserve宁大勿小，对缩小请求视而不见

---

<a id="id11"></a>
### ✅ 知识点6.2: reserve不保证精确容量

**理论**
* **可能超额分配**：实现可能分配**多于n**的空间，capacity可能大于n
* **最小保证**：capacity至少达到或超过n，不会少于请求值
* **无上限约束**：具体分配多少由实现自由决定

**注意点**
* 💡 **实际意义**：不要依赖capacity等于n，应假设capacity >= n
* 🔄 **常见策略**：多数实现采用指数增长（如翻倍），reserve(50)可能得到64或100

---

<a id="id12"></a>
## ✅ 知识点7: shrink_to_fit成员函数

**理论**
* **函数原型**（C++11起）：`void shrink_to_fit();`
* **功能**：**请求**容器将capacity减少到与size相等
* **限定**：仅适用于vector、string和deque
...
<a id="id13"></a>
### ✅ 知识点7.1: shrink_to_fit的非强制性

**理论**
* **请求而非命令**：标准明确说明这是**请求（request）**，实现可忽略
* **无保证**：调用后**不保证**capacity一定会减小
* **实现自由**：库实现可基于性能考虑拒绝收缩

**注意点**
* ⚠️ **重要限制**：`shrink_to_fit()`可能什么都不做，不能依赖其效果
* 💡 **实际行为**：多数实现会尊重请求，但可能保留一定余量
* 🔄 **与reserve对比**：reserve是强制的（满足条件时），shrink_to_fit是建议性的

---

<a id="id14"></a>
## ✅ 知识点8: size与capacity的核心区别
...
**注意点**
* 🎯 **本质区别**：size是**逻辑大小**，capacity是**物理容量**
* 💡 **类比理解**：size像杯中的水量，capacity像杯子的总容量
...
**注意点**
* 💡 **直接体现**：size是容器接口层面的"可见"大小
* 🔄 **与capacity无关**：size只反映元素数量，不涉及内存管理
...
* **变化方式**：通过reserve、shrink_to_fit或自动重分配时改变
* **性能意义**：决定何时触发昂贵的重分配操作

**注意点**
* ⚠️ **容量耗尽**：当`size() == capacity()`时，下次插入必然触发重分配
* 💡 **性能调优**：capacity越大，重分配频率越低，但内存占用越多

---

<a id="id17"></a>
## ✅ 知识点9: size与capacity交互的代码示例

**理论**
* 通过一个完整示例演示vector从空到满的size和capacity变化
* 使用循环添加24个元素，观察capacity增长模式
...
**理论**
* **size**：空vector的size为**0**，没有元素
* **capacity**：标准不规定，但多数实现将空vector的capacity设为**0**
* **内存占用**：初始时可能不分配任何内存

**注意点**
* 💡 **常见实现**：libstdc++和libc++都将空vector的capacity设为0
