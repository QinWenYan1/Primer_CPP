...
* [知识点1.1: 添加元素操作总览](#id2)：Table 9.5列出的所有添加元素操作
    ...
    * [知识点1.1.3: emplace构造操作](#id5)：直接在容器内构造元素
    * [知识点1.1.4: 返回值与迭代器管理](#id6)：insert操作的返回值及应用
...
**理论**
`insert`成员允许在容器任意位置插入元素，支持多种形式：
- `c.insert(p, t)`：在迭代器p前插入单个元素t
- `c.insert(p, n, t)`：在p前插入n个值为t的元素
- `c.insert(p, b, e)`：在p前插入迭代器范围`[b, e)`内的元素
- `c.insert(p, il)`：在p前插入初始化列表il中的元素
...
**注意点**
* ⚠️ 迭代器b和e不能指向要插入的同一容器
* ⚠️ 在vector/string中间插入代价很高，需要移动元素
* 💡 insert总是将元素插入到给定位置之前

---

<a id="id5"></a>
### ✅ 知识点1.1.3: emplace构造操作

**理论**
C++11引入的三个新成员`emplace_front`、`emplace`和`emplace_back`直接在容器管理的内存中构造元素，而非拷贝元素。它们分别对应`push_front`、`insert`和`push_back`。
...
**注意点**
* 💡 emplace函数将参数转发给元素类型的构造函数，在容器内存中直接构造对象
* 💡 push/insert操作传递的是已构造的对象，会发生拷贝
* ⚠️ emplace参数必须与元素类型的某个构造函数匹配
...
**理论**
新版标准中，insert操作返回指向第一个插入元素的迭代器。`insert(p, t)`返回指向新插入元素的迭代器。利用此返回值可反复在指定位置插入。
...
**注意点**
* 💡 insert返回指向新插入元素的迭代器，将其赋给iter可实现头插效果
* ⚠️ 调用insert后需调整迭代器位置，否则可能陷入无限循环
* 💡 erase返回指向被删元素后一个元素的迭代器，无需额外递增
...
**理论**
当初始化容器或向容器插入对象时，放入容器的是对象值的副本，而非对象本身。容器内元素与源对象之间无关联，后续修改互不影响。
...
**理论**
Table 9.6列出了顺序容器的元素访问操作。这些操作仅适用于string、vector、deque和array。forward_list不提供back()，所有容器都提供front()。

**操作汇总**
| 操作 | 返回值 | 适用容器 | 空容器行为 |
|------|--------|----------|------------|
| c.front() | 首元素引用 | 所有 | 未定义 |
| c.back() | 尾元素引用 | 除forward_list | 未定义 |
| c[n] | 位置n元素引用 | string/vector/deque/array | 未定义 |
| c.at(n) | 位置n元素引用 | string/vector/deque/array | 抛出异常 |
...
**理论**
`c.front()`返回首元素的引用，`c.back()`返回尾元素引用。所有顺序容器（包括array）都有front，除forward_list外都有back。操作前必须检查容器非空。
...
**注意点**
* ⚠️ 访问空容器的front/back是严重编程错误
* 💡 可使用begin/end迭代器间接访问，但back()更直接
* ⚠️ 对forward_list不能递减end()迭代器，不支持back()
...
**理论**
提供快速随机访问的容器(string, vector, deque, array)支持下标运算符`c[n]`和`c.at(n)`。下标不检查范围，at()在越界时抛出`out_of_range`异常。
...
**注意点**
* ⚠️ 使用越界下标是严重错误，编译器无法检测
* 💡 需要安全检查时使用at()，性能关键且能保证范围正确时使用[]
* 💡 array也支持下标和at，这是其与内置数组的重要区别
...
**理论**
访问成员(front, back, 下标, at)返回引用。const对象返回const引用，非const对象返回普通引用，可用于修改元素值。

**注意点**
* 💡 使用auto&获取引用以修改元素，使用auto获取副本
...
**理论**
Table 9.7列出了从顺序容器删除元素的操作。这些操作改变容器大小，array不支持。forward_list有特殊的erase版本。
...
<a id="id13"></a>
### ✅ 知识点3.1: pop_front和pop_back

**理论**
`pop_front()`删除首元素，`pop_back()`删除尾元素。两者都返回void。需要保存值时必须先获取再删除。
...
**注意点**
* ⚠️ vector和string不支持pop_front
* ⚠️ forward_list不支持pop_back
* ⚠️ 对空容器执行pop是未定义行为
* 💡 pop操作不返回被删值，需手动先获取
...
**理论**
`erase(p)`删除迭代器p指向的单个元素，`erase(b, e)`删除范围元素。两者都返回指向被删元素后一个位置的迭代器。
...
**注意点**
* 💡 erase返回的迭代器自动指向下一个元素，无需额外递增
* ⚠️ 删除deque首尾之外的元素会使所有迭代器失效
* ⚠️ 删除vector/string中某点后的元素会使后续迭代器失效
