...
* [*知识点5: 数值转换(C++11)*](#id13)：string与算术类型之间的相互转换
...
**理论**
* 字符形式的数字和真正的数值在存储方式和含义上完全不同
  * 数值 15 在内存中直接以二进制形式存储，而字符串中的每个字符都有各自的编码
* C++11 提供了一组标准函数，在 `string` 和数值类型之间互相转换

**技术规范**

* **数值 → 字符串**:支持所有算术类型，包括浮点和整型（int及以上），小整型会被提升

  * `to_string(val)`
  * `val`：任意算术类型
* **字符串 → 整数**
  * `stoi(s, p, b) / stol(s, p, b) / stoll(s, p, b) / stoul(s, p, b) / stoull(s, p, b)`
  * 从字符串开头解析数字，遇到非数字停止
  * `p`：保存第一个非数字字符位置（可为 `nullptr`）
    * 指向`size_t`的指针，存储第一个非数字字符的索引，默认为0（不存储）
    * 也就是告诉你解析的结束点
  * `b`：进制，默认 `10`
* **字符串 → 浮点数**
...
**代码解析**
* 在把字符串转换成数值时，字符串中第一个“非空白字符”必须是一个“可以出现在数字里的字符”
* 字符串转数值时会先忽略前导空白，第一个非空白字符必须是 `+`、`-` 或数字。
* 浮点数字符串可以以 `.` 开头，并可包含 `e/E` 表示指数。
* 整数字符串可按进制解析，如 `0x/0X` 表示十六进制，且可包含字母表示 9 以上的数。
...
* ⚠️ **起始字符**：转换的`string`必须以有效数字表示形式开头，否则抛出异常
