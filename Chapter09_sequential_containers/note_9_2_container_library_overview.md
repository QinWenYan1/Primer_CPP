...
* [*知识点2: 通用容器操作*](#id2)：所有容器类型共有的操作
  * [*知识点2.1: 类型别名*](#id3)：容器定义的类型成员
  * [*知识点2.2: 构造函数*](#id4)：容器的构造方式
  * [*知识点2.3: 赋值和swap*](#id5)：容器内容的替换和交换
  * [*知识点2.4: 大小操作*](#id6)：获取容器大小的操作
...
* `size_type`：无符号整数类型，足够大以容纳该容器类型的最大可能大小
* `difference_type`：有符号整数类型，足够大以容纳两个迭代器之间的距离
* `value_type`：元素类型
...
**理论**
* `C c;`：默认构造函数，创建空容器（array除外）
* `C c1(c2);`：构造c1作为c2的副本
* `C c(b, e);`：从迭代器b和e表示的范围复制元素（对array无效）
* `C c{a,b,c...};`：列表初始化c

**注意点**
* ⚠️ array的默认构造函数不会创建空容器
* 🔄 范围构造函数允许从不同类型的容器复制元素
...
**理论**
* `c1 = c2`：用c2中的元素替换c1中的元素
* `c1 = {a,b,c...}`：用列表中的元素替换c1中的元素（对array无效）
* `a.swap(b)`：交换a和b中的元素
* `swap(a, b)`：等价于a.swap(b)

**注意点**
* ⚠️ array支持赋值但不支持初始化列表赋值
* 💡 swap操作通常比复制元素快得多
...
**理论**
* `c.size()`：c中的元素数量（对forward_list无效）
* `c.max_size()`：c可以容纳的最大元素数量
* `c.empty()`：如果c有任何元素返回false，否则返回true

**注意点**
* ⚠️ forward_list不提供size()操作
* 💡 empty()通常比检查size() == 0更高效
...
* 所有标准容器类型的迭代器都允许我们访问容器中的元素，都提供解引用运算符
* 所有库容器的迭代器都定义递增运算符来移动到下一个元素

**注意点**
* ⚠️ forward_list迭代器不支持递减(--)运算符
* 🔄 迭代器算术运算（表3.7）仅适用于string、vector、deque和array的迭代器
...
**理论**
* **迭代器范围**由一对迭代器表示，每个迭代器引用同一容器中的元素或最后一个元素之后的位置
* 这两个迭代器通常称为begin和end，标记容器中的元素范围
* end迭代器从不引用范围内的最后一个元素，而是引用最后一个元素之后的位置

**注意点**
* 🎯 迭代器范围的概念是标准库的基础
* ⚠️ 编译器不能强制执行迭代器范围的要求，需要程序员确保遵循约定
...
* 元素范围称为**左包含区间**
* 标准数学表示法：[begin, end)
* 表示范围从begin开始，到end结束但不包括end
* begin和end必须引用同一容器
* end可以等于begin，但不能引用begin之前的元素
...
**左包含区间的便利属性**
* 如果begin等于end，范围为空
* 如果begin不等于end，范围内至少有一个元素，begin引用该范围内的第一个元素
* 我们可以递增begin若干次直到begin == end

**注意点**
* 💡 这些属性使得处理元素范围的循环可以安全编写
* 🔄 确保循环最终会终止
