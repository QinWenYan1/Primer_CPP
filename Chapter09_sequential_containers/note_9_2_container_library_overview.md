...
| `seq.assign(b, e)`             | 用迭代器区间 `[b, e)` 的元素替换 `seq` 的内容 | 该区间的迭代器 `b`、`e` 不得指向 `seq`；`assign` 不适用于关联容器和 `array`；会使目标容器的迭代器/引用/指针失效   |
...
* 顺序容器（除了`array`）还定义了一个名为`assign`的成员，允许我们从**不同但兼容**的类型赋值，或从容器的**子序列**赋值
