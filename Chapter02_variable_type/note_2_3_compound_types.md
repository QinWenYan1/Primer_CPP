...
int i = refVal; //初始化i为refVal绑定的对象值上
```
...
**类型匹配**：指针类型必须与所指对象类型匹配，因为指针类型常常用于推断所指的的对象类型， 否者操作会失败：
...
- 有些时候很难追踪是否赋值改懂了指针或所指对象，**但我们记住赋值改变的永远是左边运算数**
...
```
- 我们可以将 `void*` 与其它指针**比较**，**传入函数或返回**，还可以把各种类型地址**赋值**给 `void*` 类型的变量
...
- 声明器可以在同一个变量定义里面连同基础类型去定义一个或多个不同的复合类型的变量
...
⚠️ 这两个定义的基础类都是`int`，但是 `*` 修改了 `p1` 和 `p`的类型
