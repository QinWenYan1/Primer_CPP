...
**理论**
* 除了继承自iostream的操作外，fstream类型还添加了管理关联文件的成员
...
fstream fstrm("file.txt"); // 创建并打开文件
fstream fstrm("file.txt", ios::app); // 以指定模式打开

fstrm.open("file.txt"); // 打开文件
fstrm.close();          // 关闭文件
bool is_open = fstrm.is_open(); // 检查文件状态
```

**注意点**
* ⚠️ 接受文件名的构造函数是`explicit`的
* 💡 `is_open()`返回bool值，指示文件是否成功打开且未关闭
...
**理论**
* 当要读写文件时，需要定义文件流对象并将其与文件关联
* 每个文件流类都定义了名为`open`的成员函数，执行系统特定的操作来定位并打开文件
* 创建文件流时可以（可选地）提供文件名，此时会自动调用`open`
...
**理论**
* 可以在需要原始类型对象的地方使用继承类型的对象
* 这意味着接受iostream类型引用（或指针）的函数可以代表相应的fstream类型调用
...
* 由于open可能失败，通常需要验证是否成功
* 文件流一旦打开就保持与指定文件的关联
* 对已打开的文件流调用open会失败并设置failbit
