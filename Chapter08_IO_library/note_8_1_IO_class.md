...
* `tie()`函数用于管理流关联关系，有两种重载版本：
  1. 无参：`x.tie()` 返回当前绑定的 `ostream*`（没绑定返回 `nullptr`） 
  2. 一参：`x.tie(&o)` 把 `x` 绑定到 `o`，之后每次读 `x` 都会先 `flush(o)`，返回之前绑定的 `ostream*`
...
**注意点**
* 💡 交互式系统通常**应该**关联输入流和输出流，确保提示信息在读取前显示
* ⚠️ 每个流最多**只能**关联一个输出流，但多个流**可以**关联到同一个输出流
