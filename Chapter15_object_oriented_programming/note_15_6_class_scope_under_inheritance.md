...
**理论**
* 每个类定义自己的**作用域(scope)**，在其作用域内定义成员
* 在继承体系下，**派生类(derived class)**的作用域**嵌套(nested)**在其**基类(base class)**的作用域内
* **名字查找(name lookup)规则**：如果在派生类作用域内某个名字未解析成功，编译器会继续在外层包围的基类作用域中搜索该名字的定义
...
**注意点**
* ⚠️ ** surprising的事实**：尽管基类和派生类在程序文本中定义在不同部分，但派生类作用域确实嵌套在基类作用域内
* 💡 **理解技巧**：将继承想象成"俄罗斯套娃"结构，内层（派生类）可以看到外层（基类），但反之不行
* 📋 **术语提醒**：**scope(作用域)**、**derived class(派生类)**、**base class(基类)**、**nested(嵌套)**

---

<a id="id2"></a>
## ✅ 知识点2: 名字解析示例：isbn的查找过程
...
* 📋 **术语提醒**：**name lookup(名字查找)**、**resolved(解析)**
