...
**理论**
* 本节作为继承(inheritance)的最终示例，扩展了**§12.3**（第484页）中的文本查询(text-query)应用程序
* 原始类只能查找给定单词在文件中的出现位置
* 目标是扩展系统以支持更复杂的查询(complicated queries)

**注意点**
* 💡 **学习重点**：这是综合运用面向对象设计原则（继承、多态、抽象基类）的完整案例
* 📋 **术语对照**：text-query application（文本查询应用程序），inheritance（继承）
...
* 查找匹配给定**string**的所有行
* 示例：查询 **Daddy**
* 输出显示：Daddy出现3次，并列出包含该词的行（第2、7、10行）

**2. 非查询(Not queries)**
* 使用 **~** 运算符(operator)，返回不匹配查询的行
* 示例：**~(Alice)**
* 输出显示：~(Alice)出现9次，列出不包含Alice的行（第2、3、4行等）

**3. 或查询(Or queries)**
* 使用 **|** 运算符，返回匹配两个查询中任意一个的行
* 示例：**(hair | Alice)**
* 输出显示：(hair | Alice)出现2次，列出包含hair或Alice的行（第1、3行）

**4. 与查询(And queries)**
* 使用 **&** 运算符，返回同时匹配两个查询的行
* 示例：**(hair & Alice)**
* 输出显示：(hair & Alice)出现1次，列出同时包含hair和Alice的行（第1行）

**注意点**
* 📋 **运算符对照**：Not（非）使用 **~**，Or（或）使用 **|**，And（与）使用 **&**
* ⚠️ **输出格式**：系统会打印查询表达式，使用括号(parentheses)表明查询的解释方式，按升序(ascending order)显示行号，且**不重复显示**同一行
...
* **使用正常的C++优先级规则(precedence rules)**（§4.1.2，第136页）来评估复合表达式(compound expressions)
...
* 🔄 **知识关联**：参考 §4.1.2（第136页）复习C++运算符优先级
...
**理论**
* **初步想法**：使用 **§12.3.2**（第487页）的 **TextQuery** 类表示单词查询，并从该类派生其他查询
* **设计缺陷**：这种设计是有缺陷的(flawed)
* **原因分析**：
  * 考虑非查询(Not query)：单词查询(Word query)查找特定单词，但非查询需要能够识别被否定的单词
  * 一般情况下，不存在这样的单词。相反，非查询包含一个查询（单词查询或其他任何类型的查询），并对其结果取反(negates)
  * 类似地，与查询(And query)和或查询(Or query)包含两个查询，并组合它们的结果

**核心观察**
* 这一观察表明，应将不同类型的查询建模为**独立的类(independent classes)**，它们共享一个**共同的基类(common base class)**
...
* 📋 **关键区别**：Word query查找单词 vs Not query对任意查询结果取反
...
* 这些类将只有两个操作：
  ...
  2. **rep**：返回底层查询的 **string** 表示(representation)。该函数被 **eval** 用于创建表示匹配的 **QueryResult**，也被输出运算符(output operator)用于打印查询表达式

**抽象基类(Abstract Base Class)定义**
* 四个查询类型不是通过继承相关的，它们是概念上的兄弟(conceptually siblings)
* 每个类共享相同的接口(interface)，这表明需要定义一个**抽象基类(abstract base class)**（§15.4，第610页）来表示该接口
* 抽象基类命名为 **Query_base**，表明其作用是作为查询层次结构的根(root)

**纯虚函数(Pure Virtual Functions)**
* **Query_base** 类将 **eval** 和 **rep** 定义为**纯虚函数(pure virtual functions)**（§15.4，第610页）
* 每个表示特定查询类型的类都必须**覆盖(override)**这些函数
* **WordQuery** 和 **NotQuery** 直接从 **Query_base** 派生

**注意点**
* 📋 **术语提醒**：abstract base class（抽象基类），pure virtual functions（纯虚函数），override（覆盖/重写）
...
**理论**
* **AndQuery** 和 **OrQuery** 类共享一个其他类不具备的属性：每个都有**两个操作数(two operands)**
...
* 🔄 **继承链**：AndQuery/OrQuery → BinaryQuery → Query_base
...
**理论**
* 继承层次结构的设计本身是一个复杂的主题，超出了本书范围。但有一个重要的设计准则(design guide)非常基础，每个程序员都应该熟悉：
* **"Is A"关系**：当定义一个类公开继承自另一个类时，派生类应该反映与基类的"Is A"关系。在设计良好的类层次结构中，公开派生类的对象可以在需要基类对象的任何地方使用。
* **"Has A"关系**：类型之间的另一种常见关系是"Has A"关系。通过"Has A"关系相关的类型意味着成员资格(membership)。
* 在书店示例中，基类代表以规定价格出售的书籍报价概念。Bulk_quote"是一种(Is A)"报价，但具有不同的定价策略。书店类"有一个(Has A)"价格和一个ISBN。

**注意点**
* 💡 **核心设计原则**：继承表示"Is A"（是一种），组合/成员表示"Has A"（有一个）
* ⚠️ **常见错误**：滥用继承（如让Not query继承Word query）会破坏"Is A"语义
...
**理论**
* 程序将处理评估查询(evaluating queries)，而不是构建查询(building them)。然而，为了能够运行程序，需要能够创建查询
* 最简单的方法是编写C++表达式来创建查询。例如，希望通过编写如下代码生成前面描述的复合查询：
```cpp
Query q = Query("fiery") & Query("bird") | Query("wind");
```

**接口类(Interface Class)设计**
* 问题描述隐含表明用户级代码(user-level code)不会直接使用继承的类
