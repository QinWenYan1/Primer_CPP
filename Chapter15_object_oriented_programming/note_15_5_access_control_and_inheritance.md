...
* **public继承**：基类成员在派生类中保持原有访问级别，后续派生类可以继续访问`protected`成员
* **private继承**：基类成员在派生类中变为private，**后续派生类无法访问**这些继承成员

**注意点**
* ⚠️ **连锁效应**：`Priv_Derv`的派生类（`Derived_from_Private`）无法访问`prot_mem`，因为`Priv_Derv`使用`private`继承使所有`Base`成员变为了`private`
* 💡 **设计考虑**：如果你希望阻止后续派生类访问基类的`protected`成员，使用`private`继承
...
* **派生类到基类转换**是否可访问取决于：
  ...
  2. 派生类使用的**派生访问说明符**
* **转换规则**：
  * **用户代码想使用转换**：只有当D**public继承**自B时，才能使用D到B的转换；`protected`或`private`继承时不能使用
  * **派生类D的成员和友元可以使用转换**：无论D如何继承自B，都可以使用转换；转换对直接基类总是可访问的
  * **派生类D的派生类的成员和友元可以使用转换**：只有当D<b>`public`或`protected`继承</b>自B时才能使用；`private`继承时不能使用
* **规则核心逻辑**:
  * 防止在派生类不对外的接口在转换后又对外开放的以外暴露风险

**注意点**
* 📋 **关键记忆点**：对于代码中任何给定的点，如果基类的`public`成员可访问，则派生类到基类的转换也可访问；否则不可访问
* 🔄 **与成员访问的关联**：转换的可访问性与继承后基类`public`成员的可访问性一致

---

<a id="id8"></a>
## ✅ 知识点8: 关键概念：类设计与`protected`成员

**理论**
* **三种用户视角**：
  1. **普通用户**：编写使用类对象的代码，只能访问`public`成员（接口）
  2. **实现者**：类的成员和友元，可以访问`public`和`private`成员（实现细节）
  3. **派生类**：继承体系中的第三类用户
* **`protected`的设计哲学**：
  * 基类将愿意让派生类使用的实现部分声明为`protected`
  * `protected`成员对普通用户代码不可访问，但对派生类及其友元可访问
  * `private`成员对派生类保持不可访问
* **设计准则**：

  * 如果派生类在实现自己的功能时，需要直接调用基类的某个内部工具（函数）或使用基类的内部数据，<b>那这个成员就该设为 `protected`</b>，像"传家宝"一样开放给子孙后代继承使用。
  * 否则，它就应该严格设为 `private`，像"个人隐私"一样完全对外（包括派生类）隐藏，避免被误用或依赖。
...
* **例外情况**：但当基类把自己"嵌入"进派生类对象里时（作为子对象），这个死党依然能认出基类并访问其隐私，哪怕基类藏在派生类内部。
* **`friendship`声明范围**：
  * 当一个类声明另一个类为友元时，**仅该类本身**获得访问权限
  * 其基类和派生类都不会自动获得特殊访问权
...
```
**注意点**
* ⚠️ **常见陷阱**：`f3`合法但`f2`非法——因为`Pal`是`Base`的友元（可以访问`prot_mem`），但不是`Sneaky`的友元（不能访问j）
---

<a id="id10"></a>
## ✅ 知识点10: 使用`using`声明豁免个别成员

**理论**
* <b>`using`声明</b>可以改变派生类继承而来的名字的访问级别
* **机制**：使用`using`声明可以将基类成员"提升"或"调整"到派生类中的指定访问区域
* **使用场景**：当使用`private`继承（默认）但希望某些继承成员保持`public`或`protected`访问级别时特别有用
* **访问级别决定**：`using`声明所在的位置前的访问说明符决定了该名字的访问级别
  * `private`部分：仅对成员和友元可访问
  * `public`部分：对所有用户可访问
  * `protected`部分：对成员、友元和派生类可访问
* **限制**：派生类只能对**允许访问**（即非`private`）的名字提供`using`声明
...
* 🔄 **效果**：`Derived`的使用者可以访问`size`成员，从`Derived`继续派生的类可以访问n成员
* ⚠️ **using与继承**：using声明可以命名任何可访问的**直接或间接基类**成员
...
* **默认派生访问说明符**取决于定义派生类使用的关键字：
  * 使用<b>`class`</b>关键字：默认是**私有继承**
  * 使用<b>`struct`</b>关键字：默认是**公有继承**
* **常见误解澄清**：`struct`和`class`定义类**没有本质区别**，仅有的两个区别是：
  1. 默认成员访问说明符（`struct`默认`public`，`class`默认`private`）
  2. 默认派生访问说明符（`struct`默认`public`，`class`默认`private`）
...
**注意点**
* 📋 **一致性**：默认行为与成员访问一致——`struct`偏向"开放"(`public`)，`class`偏向"封闭"(`private`)
* ⚠️ **潜在陷阱**：混用`struct`和`class`时容易混淆默认继承方式，**建议始终显式指定**
...
**理论**
* **最佳实践**：使用私有继承的类应**显式指定`private`**，而非依赖默认行为
* **理由**：显式声明使代码意图清晰，表明`private`继承是**有意为之**，而非**疏忽**

**注意点**
* 💡 **代码可读性**：显式访问说明符让其他程序员无需记住默认规则就能理解代码
* 🔄 **维护性**：即使后续将`struct`改为`class`（或反之），显式指定的继承方式也不会意外改变
