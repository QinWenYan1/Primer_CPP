...
**理论**
* `equal`**算法有一个关键假设**：第二个序列至少与第一个序列一样大，否则会有严重的程序错误
...
**理论**
* 常见的初学错误是在**空容器**上调用`fill_n`（或其他写元素的算法）
...
* 接受**容器的引用**，返回**绑定到该容器的插入迭代器**
    * 插入迭代器不是位置迭代器，不指向位置，只**绑定容器**
    * 也就是没有`begin`, `end`概念
    * 通过插入迭代器赋值时，赋值操作**会调用**容器的`push_back`添加元素
* 这是确保算法有足够元素容纳输出的一种方法
* 可以将`fill_n`和`back_iterator`结合起来进行连续的`push_back`而不覆盖元素
...
```
**代码解析**
* 因为 `back_inserter` 不是“位置”，而是一种“每次赋值就调用 `push_back` 的写入行为”
* 所以 `fill_n` 的每一次赋值都会触发一次插入，而不存在“先定位、再连续赋值”


**注意点**
* 💡 `back_inserter`创建的迭代器每次赋值**都会调用**`push_back`，自动扩展容器
