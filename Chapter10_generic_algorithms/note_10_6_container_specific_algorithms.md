...
* **成员算法的必要性**：
    ...
    2.  **性能（针对其他算法）**：对于`merge`、`remove`等操作，通用算法通过**交换元素值**实现，对链表效率低。链表专用的成员算法则通过**修改节点间的链接(links)**来操作，无需移动数据，**性能远优于通用版本**。

**注意点**
* ⚠️ **关键限制**：**绝不能对`list`或`forward_list`使用通用`std::sort`算法**。
* 📋 **术语提醒**：迭代器种类：随机访问迭代器(random-access iterator)、双向迭代器(bidirectional iterator)、前向迭代器(forward iterator)。

---

<a id="id3"></a>
## ✅ 知识点3: Table 10.6 - list和forward_list的成员算法

**理论**
* 下表列出了`list`和`forward_list`定义的成员算法。这些操作**返回void**。其他能接受合适迭代器的通用算法在链表上效率与其他容器相当。
...
**注意点**
* 📋 **术语提醒**：splice (v. 拼接)，形象描述了连接两段链表的操作。
