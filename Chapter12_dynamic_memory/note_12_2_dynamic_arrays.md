...
**理论**
* **核心主旨总结**：`new`和`delete`一次只能分配/释放一个对象。为了支持需要一次性为多个对象分配连续内存的场景（如`vector`、`string`重新分配时），C++语言和库提供了两种分配对象数组的方式：`new`的数组形式，以及`allocator`模板类。
* **关键要点**：
    * `allocator`类可以将内存**分配(allocation)**与对象**构造(construction)**分离，通常能提供更好的性能和更灵活的内存管理。
    * **最佳实践(Best Practices)**：**大多数应用程序应该使用库容器（如`vector`），而不是动态分配的数组**。使用容器更容易、更不容易出现内存管理错误，并且可能提供更好的性能。
    * 使用容器的类可以使用默认的拷贝、赋值和析构操作。而分配动态数组的类必须自定义这些操作来管理关联的内存。
* **警告(Warning)**：**在阅读第13章之前，不要在类的代码内部分配动态数组。**
...
**理论**
* **核心主旨总结**：通过在类型名后跟一对方括号，并在括号内指定要分配的对象数量，可以让`new`分配一个对象数组。`new`会分配请求数量的对象，并（假设分配成功）返回指向第一个对象的指针。
* **关键要点**：
    * 方括号内的**大小(size)**必须是整数类型，但不必是常量。
    * 也可以使用**类型别名(type alias)**来表示数组类型，此时在`new`中可以省略方括号。但编译器仍会按`new[]`的方式执行。
* **术语提醒**：动态数组(dynamic array)、类型别名(type alias)。
...
**理论**
* **核心主旨总结**：使用`new T[]`分配内存通常被称为“动态数组”，但这有些误导。**我们得到的并不是一个数组类型的对象，而是一个指向数组元素类型的指针**。即使使用数组类型别名，`new`也不会分配数组类型的对象。
* **关键后果**：
    * 因为分配的内存没有数组类型，所以我们**不能**在动态数组上调用`begin()`或`end()`。这些函数需要使用数组维度（数组类型的一部分）。
    * 同样原因，我们也**不能**使用**范围for循环(range for)**来处理所谓的动态数组的元素。
* **术语提醒**：元素类型(element type)、数组类型(array type)、范围for循环(range for)。
...
* **关键要点**：
    ...
    * 分离分配和构造允许我们分配大块内存，只在真正需要创建对象时才支付对象构造的开销。
* **术语提醒**：分配(allocation)、构造(construction)、析构(destruction)、释放(deallocation)、模板类(template class)。
...
**理论**
* **核心主旨总结**：`allocator`对象分配的内存是**未构造的(unconstructed)**。我们通过在该内存中构造对象来使用它。使用完毕后，必须先销毁构造的对象，然后才能释放内存。
* **关键操作**：
    1.  `allocate(n)`：分配能保存`n`个类型为`T`的对象的原始、未构造内存。
    2.  `construct(p, args)`：在指针`p`指向的原始内存中，使用参数`args`构造一个类型为`T`的对象。
    3.  `destroy(p)`：对指针`p`指向的对象运行析构函数。
    4.  `deallocate(p, n)`：释放从指针`p`开始的内存，该内存必须是由`allocate`分配的，且大小参数`n`必须与分配时一致。
* **重要警告**：
    * ⚠️ **必须构造对象后才能使用`allocate`返回的内存**，以其他方式使用未构造的内存是未定义行为。
    * ⚠️ **只能销毁实际已构造的元素**。
    * 传递给`deallocate`的指针不能是`nullptr`，且大小必须匹配。
* **术语提醒**：未构造的(unconstructed)、原始内存(raw memory)、析构函数(destructor)。
...
**理论**
* **核心主旨总结**：标准库在`<memory>`头文件中定义了几个算法，作为`allocator`类的补充，用于在未初始化的内存中构造对象。这些算法**构造(elements)**元素，而不是给它们赋值。
* **关键算法**：
...
* **术语提醒**：未初始化的内存(uninitialized memory)、迭代器(iterator)、目的位置(destination)。
...
**注意点**
* 🔄 **知识关联**：`uninitialized_copy`类似于`copy`算法，但它在目的地构造对象。`uninitialized_fill_n`类似于`fill_n`算法，但它是在原始内存中构造对象。
