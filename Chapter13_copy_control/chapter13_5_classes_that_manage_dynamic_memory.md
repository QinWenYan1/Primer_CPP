...
**理论**
* **核心主旨总结**：某些类需要在运行时分配可变大小的存储空间，通常应优先使用标准库容器（如 `vector`）来管理数据。例如 `StrBlob` 类使用 `vector` 管理底层存储。但并非所有类都能采用这种策略，**必须自己进行内存分配的类，通常需要定义拷贝控制成员**来正确管理所分配的内存。
* **保持技术准确性**：这里强调的是**拷贝控制成员(copy-control members)**（拷贝构造函数、拷贝赋值运算符、析构函数）在管理动态内存时的必要性。
* **重点突出**：**如果可以，应优先使用库容器**；只有在必须自定义内存管理时，才需要实现拷贝控制。
...
* 💡 **理解技巧**：将“拷贝控制成员”看作类与动态内存之间的契约，确保分配与释放成对出现。
...
* **核心主旨总结**：本节实现一个简化版的 `vector`，它专用于存储 `string`，因此命名为 `StrVec`。设计策略模仿 `vector`：
  - 元素存储在**连续内存(contiguous storage)**中。
  - 为获得可接受的性能，会**预分配(preallocate)**多于实际需要的空间。
...
**注意点**
* 📋 **术语提醒**：**预分配(preallocate)**、**重新分配(reallocation)**、**分配器(allocator)**。
...
**注意点**
* ⚠️ **警告注意**：类定义中只声明了拷贝控制成员，未定义它们，实现将在后面给出。
* 📋 **术语提醒**：`allocator` 是标准库提供的用于分配未构造内存的类模板。
* 💡 **理解技巧**：三个指针的差分别表示**已用元素个数**（`first_free - elements`）和**容量**（`cap - elements`）。
...
**注意点**
* ⚠️ **警告注意**：`alloc` 成员是默认初始化的，不需要在初始化列表中显式写出，但其默认构造函数会被自动调用。
* 💡 **理解技巧**：空 `StrVec` 的三个指针均为 `nullptr`，此时 `size()` 和 `capacity()` 均为 0。
...
* 📋 **术语提醒**：**尾后指针(off-the-end pointer)**，这里 `first_free` 即扮演此角色。
...
**注意点**
* 🔄 **知识关联**：`size()` ≤ `capacity()`，当两者相等时，添加新元素将触发重新分配。
* 💡 **理解技巧**：`cap - elements` 是总容量，`first_free - elements` 是已用量。
...
**注意点**
* ⚠️ **警告注意**：该函数被 `push_back` 等添加元素的成员调用，是**重新分配策略**的入口。
* 💡 **理解技巧**：相当于“检查并扩容”函数。
...
**注意点**
* ⚠️ **警告注意**：这些指针在空对象时均为 `nullptr`，调用 `begin()` 或 `end()` 不会导致未定义行为，但解引用需要先检查非空。
* 📋 **术语提醒**：这里返回的是**普通指针(raw pointer)**，但行为上充当**迭代器(iterator)**。
...
* 📋 **术语提醒**：**后置递增(postfix increment)** 运算符，先取当前值，再递增。
...
**理论**
* **核心主旨总结**：`alloc_n_copy` 在拷贝或赋值时被调用，它**分配足够容纳给定范围元素的内存**，并将该范围内的元素拷贝到新分配的内存中。它返回一个 `pair<string*, string*>`，`first` 指向新分配内存的起始位置，`second` 指向最后一个构造元素之后的位置（由 `uninitialized_copy` 返回）。
...
* 📋 **术语提醒**：`uninitialized_copy` 是标准库算法，用于在未初始化内存区域构造对象副本。
* 💡 **理解技巧**：该函数**一次性完成分配+构造**，返回两个指针标记内存区间，类似“复制整个内存块”。
...
**理论**
* **核心主旨总结**：`free` 负责**销毁已构造的元素**并**释放整个内存空间**。销毁按**逆序(reverse order)**进行（从最后一个元素到第一个），通过调用 `alloc.destroy` 逐个执行 `string` 的析构函数。然后调用 `alloc.deallocate` 释放整块内存。函数首先检查 `elements` 非空，避免对空指针调用 `deallocate`。
...
* 💡 **理解技巧**：逆序销毁有助于与构造顺序对称，但对 `string` 并非强制要求，只是常见做法。
* 🔄 **知识关联**：`free` 在析构函数和拷贝赋值运算符中被调用，确保资源正确释放。
...
**理论**
* **核心主旨总结**：拷贝构造函数调用 `alloc_n_copy` 为源对象的所有元素分配新内存并拷贝构造。返回的 `pair` 中 `first` 赋给 `elements`，`second` 同时赋给 `first_free` 和 `cap`，因为新分配的内存大小恰好等于元素个数，所以容量等于已用大小。
...
**注意点**
* ⚠️ **警告注意**：当 `StrVec` 对象生命周期结束时，析构函数自动调用，确保没有内存泄漏。
* 💡 **理解技巧**：`free()` 已经处理了空对象情况（`elements` 为 `nullptr`），因此析构函数非常简洁。
...
**理论**
* **核心主旨总结**：拷贝赋值运算符采用**拷贝并交换(copy and swap)** 风格，但此处直接实现：先调用 `alloc_n_copy` 为右侧操作数的元素分配新内存并拷贝，然后调用 `free()` 释放左侧原有内存，最后更新三个指针。**这种顺序天然地处理了自赋值(self-assignment)**：因为先分配新内存，再释放旧内存，即使左右对象相同，也是安全的。
...
* 💡 **理解技巧**：这种实现**不需要显式检查自赋值**，且是异常安全的：如果 `alloc_n_copy` 抛出异常，旧对象状态不变。
...
**理论**
* **核心主旨总结**：`reallocate` 需要分配新内存、将现有元素转移到新空间、释放旧内存。由于 `string` 类具有**值行为(value behavior)**，拷贝一个 `string` 会分配独立字符存储，开销较大。但在重新分配时，旧元素即将被销毁，**仅有一个用户（新对象）会使用这些字符串**，因此**移动(move)** 而非拷贝这些字符串可以大幅提升性能：移动操作通常只是交换资源指针，避免字符存储的分配与释放。
...
* 📋 **术语提醒**：**值行为(value behavior)**，**移动语义(move semantics)**。
* 💡 **理解技巧**：重新分配时，旧元素已经是“将亡值”，移动比拷贝更高效。

---

<a id="id16"></a>
## ✅ 知识点16: 移动构造函数和 std::move

**理论**
* **核心主旨总结**：标准库类（如 `string`）定义了**移动构造函数(move constructor)**，它通常通过“移动”资源而非拷贝来实现。`std::move` 函数（定义在 `<utility>` 头文件中）将其参数转换为右值引用，从而触发移动构造函数。关键点：
...
* 📋 **术语提醒**：**右值引用(rvalue reference)**、**移动构造函数(move constructor)**。
...
**注意点**
* ⚠️ **警告注意**：移动后的旧字符串处于**有效但未指定**状态，调用 `free` 会对其执行析构函数，这是安全的，因为 `string` 的析构函数仅释放资源，而资源已被移动走，析构不会造成二次释放。
* 💡 **理解技巧**：`dest` 和 `elem` 的递增步调一致，分别标记新空间构造位置和旧空间读取位置。
* 📋 **术语提醒**：**扩容因子(growth factor)**，此处采用双倍策略。
