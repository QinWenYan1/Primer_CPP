...
**理论**
* 主要用于 **指针或引用类型** 的 **低层（low-level）const** 转换。
* 从 **非 const → const** 的转换是安全的，因此被编译器允许。
* 相反，**const → 非 const** 的隐式转换是不允许的（会破坏类型安全）。
...
* ⚠️ 编译器只会自动执行“**增加 const**”的转换，
  * 若要去除 const，必须显式使用 `const_cast`（**不推荐**）
...
**理论**
* 小整数类型（如 `char`、`signed char`、`unsigned char`、`short`、`bool` 等）
  在表达式求值或函数匹配时会<b>自动提升(promotion)</b>为 `int` 或更大的整数类型
* 这种提升是 **安全的（不会丢失信息）**，属于隐式类型转换的一种
* 小整数类型**总是提升**为int或者更大的整型，即使看起来与short更匹配
* 提升保证了算术运算的一致性和效率
...
<a id="id13"></a>
## ✅ 知识点5.4: 等级4-算术转换/指针转换

**理论**
* **算术转换**：int → double、long → float 等不同类型间的转换
* **指针转换**：除了**数组或函数类型**转换为**对应指针类型**这一种外的指针转换，如：
  * 常量整数0和`nullptr`可转换为任意指针类型。
  * 非`const`类型指针可转换为`void*`。
  * 以及任意类型指针可转换为`const void*`。
* **算术转换之间没有优先级差异**:
  * 在重载解析中被视为同等优先级，不存在谁比谁更"近"。
  * 容易导致调用歧义
* 设计重载函数时应避免参数类型过于接近。
...
void manip(long);
void manip(float);
manip(3.14); // 错误：都是算术转换，优先级相同，调用歧义
```

**注意点**
* ⚠️ 内置类型的自动转换可能造成重载歧义，**良好的接口设计应避免相近类型的形参**
...
* 当重载函数区别仅在于引用或指针的constness时：
  - low-level const实参**只能匹配**low-level const形参
