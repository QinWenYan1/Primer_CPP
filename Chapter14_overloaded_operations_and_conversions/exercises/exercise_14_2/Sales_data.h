...
public:
    ...
    Sales_data operator+(const Sales_data& obj1) const {
        ...
        Sales_data outcome; 
        if ( obj1.bookNo != bookNo) return outcome;
        outcome.bookNo = obj1.bookNo;  
        outcome.units_sold = obj1.units_sold + units_sold; 
        double total_price = units_sold * price + 
                            (obj1.units_sold) * (obj1.price);
        outcome.price = avg_price(total_price, outcome.units_sold);
        return outcome; 
    ...
    Sales_data& operator+=(const Sales_data& obj1) {
        ...
        units_sold = obj1.units_sold + units_sold; 
        double total_price = units_sold * price + 
                            obj1.units_sold * obj1.price;
